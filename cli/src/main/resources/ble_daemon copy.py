...
class DumbDaemon:
    def __init__(self, host='127.0.0.1', port=9000, sim_mode=False):
        ...
        # Simulator state
        self.sim_counter = random.randint(50, 200) # Start with a random historical counter
    ...
    async def handle_write(self, data):
        if self.sim_mode:
            ...
            if self.creds:
                ...
                # plaintext[0] is the incoming counter
                incoming_counter = plaintext[0]
                
                # Calculate the forward distance in a 256-value circle
                forward_distance = (incoming_counter - self.sim_counter) % 256
                
                # Define a valid sliding window (e.g., allow up to 16 dropped/missed packets)
                # Distance 0 is a direct replay of the last command.
                # Distance > 16 assumes it's an old packet wrapping around from the past.
                if forward_distance == 0 or forward_distance > 16:
                    logger.warning(f"[SIM] REJECT: Replay attack or invalid window. Current: {self.sim_counter}, Incoming: {incoming_counter}, Distance: {forward_distance}")
                    return   
    ...
    async def sim_scan_loop(self):
        while True:
            ...
            # Broadcast the current counter so the Kotlin app can read it and sync
            # We will use index 4 to hold the counter value
            mfr_bytes[4] = self.sim_counter
